...
import { Message as MessageType } from "@/types/Translate"

const Message = ({ role, text }: MessageType) => {
  const isAssistant = role === "assistant"

  const [mainText, metaText] = isAssistant ? text.split(/(?=Тематика текста:)/) : [text || "", ""]
  ...
  return (
    <div className={`flex flex-col gap-y-4 flex-1 w-full ${!isAssistant ? "items-end" : "items-start"}`}>
      {/* Сообщение */}
      <div
      ...
      >
        {isAssistant && (
          <div className="lg:min-w-[48px] w-[24px] h-[24px] lg:w-[48px] lg:h-[48px] rounded-full bg-black flex items-center justify-center">
            <Image src={navLogo} className="w-[32px] h-[32px] scale-75 lg:scale-100" alt="" />
          </div>
        )}

        <div className={`flex flex-col max-w-[80vw] ${isAssistant ? "justify-start" : "items-end w-full"}`}>
          <div
      ...
      {isAssistant && metaText && (
        <div className={`flex gap-x-[2.5vw] lg:gap-x-[2vw] w-full items-end justify-start`}>
          ...
          <div className="flex flex-col max-w-[80vw] justify-start">
            <div className="px-6 py-4 lg:p-6 max-w-[90%] md:max-w-[70%] xl:max-w-[65%] rounded-[24px] bg-BotMessageBG">
              <p className="text-PrimaryBlack text-[12px] md:text-[14px] lg:text-[16px] break-words">{metaText}</p>
            </div>
          </div>
        </div>
      )}
